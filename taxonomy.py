...
try:
    ...
    for i, elem in enumerate(efficiency_rows):
        ...
        year = elem[2]
        output = elem[3]
        output_val = str(tech) + "-" + str(year) + "-" + str(output)
        ...
        if output in OUTPUT_ELIGIBLE and int(year) >= START_YEAR:
            ...
            control_in.append(control_val)
        if (output in VALID_EMISSIONS or output in VALID_EFFICIENCY) and int(year) >= START_YEAR:
            output_control.append(output_val)
            control_out.append(control_val)

    # delete duplicates from control
    control_dump_in = []
    control_dump_out = []
    for elem in control_in:
        if elem not in control_dump_in:
            control_dump_in.append(elem)

    for elem in control_out:
        if elem not in control_dump_out:
            control_dump_out.append(elem)
    ...
    for i in input_control:
        ...
        for elem in control_dump_in:
            if check in elem:
                count = count + 1
                check_map_in.update({key: count})

    check_map_out = dict()
    for i in output_control:
        count = 0
        out_split = i.split("-")
        tech_i = out_split[0]
        year_i = out_split[1]
        output_i = out_split[2]
        check = str(tech_i) + "-" + str(year_i)
        key = str(tech_i) + "-" + str(year_i) + "-" + str(output_i)
        for elem in control_dump_out:
            if check in elem:
                count = count + 1
                check_map_out.update({key: count})
    ...
    for element in efficiency_rows:
        ...
        output = element[3]
        key = str(tech) + "-" + str(year) + "-" + str(output)
        key_check_in = str(tech) + "-" + str(year) + "-" + str(input)
        key_check_out = str(tech) + "-" + str(year) + "-" + str(output)
        str_check = "Efficiency_Premium"
        count_check_in = check_map_in.get(key_check_in)
        count_check_out = check_map_in.get(key_check_out)
        if year >= START_YEAR:
            if key not in tech_year_output_new.keys() and output in VALID_EFFICIENCY:  # not include all tech in efficiency rows
                v = "NULL"
                tech_year_output_new.update({key: v})
                if output in OUTPUT_ELIGIBLE and input in INPUT_ELIGIBLE and count_check_in == 1 and str_check not in tech_year_output_new[key]:
                    tech_year_output_new.update({key: tech_year_output_new[key] + "-" + "Efficiency_Premium"})
    ...
    # FIRST ATTEMPT: premium +26 bp = WRITE (0.26/100) !
    premium = - 0.05/100
    penalty = + 0.05/100
